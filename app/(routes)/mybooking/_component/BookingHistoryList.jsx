  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
        ...
        {bookingHistory.map((booking,index)=>(
           <div key={booking.id || index} className='border rounded-lg p-4 mb-5'>
            <div key={index} className='flex gap-4  '>
              ...
              <Image src={booking?.businessList?.images[0]?.url}
                ...
                <h2 className='flex gap-2 text-gray-500'>
                    ...
                    Service le : <span className='text-black'> {booking.date}</span></h2>
                    <h2 className='flex gap-2 text-gray-500'>
                     <Clock className='text-primary'/> 
                     Service le : <span className='text-black'> {booking.time}</span></h2>
